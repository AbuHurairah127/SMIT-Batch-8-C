...
const multiplyWithIndex = (a, b) => a * b;
...
const returnedArr6 = arr.filter((elem) => {
    if (elem > 0) {
        return true;
    }
});
...
const returnedArr7 = arr.filter((elem, index) => elem > 0);
...
// Given an array of numbers [1, 2, 3, 4, 5], use the map method to create a new array where each number is multiplied by 2.
let array = [1, 2, 3, 4, 5];
const double = (elem, index) => elem * 2;
let doubled = array.map(double);
console.log("🚀 ~ doubled:", doubled);
// Given an array of strings ["apple", "banana", "cherry", "date", "grape"], use the filter method to create a new array containing only the fruits with more than 5 characters.
let fruits = ["apple", "banana", "cherry", "date", "grape"];
const stringLen = (elem, index) => {
    if (elem.length > 5) {
        return true;
    }
};
